...
bool Block::is_loop_header() {
  assert(_loop != NULL, "does not belong to a loop");
  return _loop->head() == this;
}

// Determine if this block is the "real loop". Method level loop is not a "real loop"
bool Block::in_real_loop()  {
  return _loop != NULL && _loop->depth() != 0;
}
...
void PhaseSimpleCFG::collect_loop_members(Block_List* list, CFGElement* el) {
  if (el->is_block()) {
    list->push(el->as_Block());
  } else {
    assert(el->is_loop(), "CFGElement is neither loop nor block");
    CFGLoop* lp = el->as_CFGLoop();
    for (int i = 0; i < lp->members()->length(); i++) {
      collect_loop_members(list, lp->members()->at(i));
    }
  }
}
