...
public:
  virtual bool equals(AllocState* as) = 0;
...
public:
  ...
  virtual bool equals(AllocState* as);
...
public:
  ...
  virtual bool equals(AllocState* as) { assert(false, "not implement"); return false; }
...
private:
  ...
  Dict* _aliases;
  // Speculative block state for loop header block, this can be proved to wrong
  // In such case, I need to reprocess the whole loop. Use "stack" to process
  // nested loop
  GrowableArray<BlockState*>* _initial_loop_states;
...
private:
  ...
  void add_block_state(Block* block, BlockState* bstate) {
    ...
    guarantee(added == true, "BlockState is already exist");
  }
  BlockState* get_block_state(Block* block) {
    BlockState** bstate =  _block_states.get(block->head()->_idx);
    guarantee(bstate != NULL, "BlockState is absent");
    return *bstate;
  }
  ...
  void clear_block_state(Block* block) {
    bool removed = _block_states.remove(block->head()->_idx);
    guarantee(removed, "can not specific block");
  }
  BlockState* create_block_state(Block* b) {
    guarantee(!has_block_state(b), "block already has associated state");
...
private:
  ...
  bool is_visited_all_preds(Block* b, Block_List* visited);
  void collect_pred_block_states(Block* b, GrowableArray<BlockState*>* bstates);
